...
bool runDisplayProbePass(const EpdProbePins& p, uint8_t ver[5], uint8_t* flg, uint8_t rstLowMs) {
  ...
  if (p.rst >= 0) {
    // A deep-sleep GPIO hold survives reset and reflashing. Release it before
    // trying to pulse RESET or the probe can silently select the wrong driver.
...
DisplayControllerVerdict probeDisplayController(const EpdProbePins& p, uint8_t verBytes[5], uint8_t* flg,
  ...
  // evidence from RMTP. A programmed MTP starts with the unambiguous 0xA5 key.
  // Some field UC8279d modules instead have a blank MTP whose non-uniform dump
  // is still driven and reproducible. UC8253 has no RMTP command, so its read
  // normally floats to a uniform value; one noisy dump is not enough to match.
